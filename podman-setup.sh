...
if ! podman-compose version >/dev/null 2>&1; then
  echo "Podman Compose not available (try: podman-compose version)" >&2
  exit 1
fi
...
if [[ "$SKIP_BUILD" == false ]]; then
  ...
  if ! podman image exists "$IMAGE_NAME"; then
    echo "Error: Image $IMAGE_NAME does not exist. Run without --skip-build first." >&2
    exit 1
  fi
fi
...
echo ""
podman-compose "${COMPOSE_ARGS[@]}" run --rm openclaw-cli onboard --no-install-daemon
...
podman-compose "${COMPOSE_ARGS[@]}" up -d openclaw-gateway
